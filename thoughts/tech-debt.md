...
- [2026-06-02][PR4 follow-up] Brief-generation code still uses `triage` naming in several files/channels, which now overlaps with the broader Linear triage process. Reason: deferred-phase. Re-evaluate: after PR #4 lands; do a full pre-production rename to `brief` across internals, types, tests, and IPC channels for the cleaner long-term model. — Resolved 2026-09-02: renamed brief-generation internals (files, types, IPC channels, hooks, UI copy) to `brief`; Linear's own Triage status/tab/fetch plumbing and the shared comment-triager were deliberately left as `triage`.
