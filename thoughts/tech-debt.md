...
- [2026-06-02][PR4 follow-up] **Resolved 2026-09-02.** Brief-generation code used `triage` naming in several files/channels, overlapping with the broader Linear triage process. Renamed brief-generation internals (files, types, IPC channels, hooks, UI copy) to `brief`. Linear's own Triage status/tab/fetch plumbing and the shared comment-triager were deliberately left as `triage`.
