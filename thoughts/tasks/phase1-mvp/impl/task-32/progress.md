...
QA-fix follow-up:
- Commit message: `fix(renderer): harden atom accessibility semantics`
- `2026-05-13`
- Updated `src/renderer/components/status-dot.tsx` to add `role="img"` while preserving `aria-label={state}` so the dot is an accessible, named element.
- Updated `src/renderer/components/pill-tab.tsx` to set `type="button"` and prevent form-context submit behavior.
- Expanded `tests/renderer/atoms.test.tsx` assertions to verify:
  - `StatusDot` is exposed as role `img` with accessible name via `aria-label`.
  - `PillTab` has `type="button"`.
- Validations re-run:
